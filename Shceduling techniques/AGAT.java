        ...
        while (nonActive.size() != 0) {
            ...
            while (true) {
                ...
                if (readyQueue.size() != 0) {
                    ...
                    //if the process used all its quantum and it still has a job to do
                    ...
                    while (true) {
                        ...
                        if (currentProcess==leastFactorProcess && leastFactorProcess.getRemainingTime()>0) {
                            ...
                            //if the process didn’t use all its quantum
                            if (currentProcess.getQuantum() > v && currentProcess.getRemainingTime() > 0) {
                                readyQueue.remove(currentProcess);
                                readyQueue.add(currentProcess);
                            ...
                            //if The running process finished its job
                                ...
                                w=t-currentProcess.getBurstTime()-currentProcess.getArrivalTime();
                                System.out.println("waiting time for "+currentProcess.getName()+" = "+w);
                                ta+=w+currentProcess.getBurstTime();
                                x=w+currentProcess.getBurstTime();
                                System.out.println("turnaround time for "+currentProcess.getName()+ " = "+x);
                                waiting+=(t-currentProcess.getBurstTime()-currentProcess.getArrivalTime());

                                if (nonActive.size() == 0) {
                                    System.out.println("time: "+t);
                                    System.out.println("**********************");
                                    System.out.println("avg Waiting = "+waiting/deadList.size());
                                    System.out.println("avg turnaround time = "+ta/deadList.size());
                                    return;
                                }
                                else
