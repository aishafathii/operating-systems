...
public class ShortestJobFirst {
    ...
    public void ShortestJobFirstScheduling(){
        ...
        Process LargeProcess = RemainEdProcesses.get(RemainEdProcesses.size()-1);
        ...
        while(true){
            ...
            {
                boolean check = true;
                for(int j = 0 ; j < RemainEdProcesses.size(); j++){
                    if(LargeProcess.getName() == RemainEdProcesses.get(j).getName()){
                        check = false;
                    }
                }
                System.out.println("check = " + check);
                
                System.out.println("larger = " + LargeProcess.getName());
                
                System.out.println("first process = " + processes.get(0).getName());
                if((VarLargejob <= VarShorterJobs) && (processes.get(0).getName() != LargeProcess.getName()) && (check == false)){
                    ...
                    System.out.println("varLargerJobs = " + VarLargejob);
                        ...
                        if(RemainEdProcesses.size() > 0){
                            ...
                            LargeProcess = RemainEdProcesses.get(RemainEdProcesses.size()-1);
                        }
                }
            }
            if(!readyQueue.isEmpty()){
                ...
                boolean check = true;
                boolean findLargerProcess = false;
                int i;
                ...
                for(i = 0 ; i < RemainEdProcesses.size(); i++){
                    if(LargeProcess.getName() == RemainEdProcesses.get(i).getName()){
                        findLargerProcess = true;
                    }
                }
                if(findLargerProcess == false){
                    if(RemainEdProcesses.size() > 0){
                        VarLargejob = RemainEdProcesses.get(RemainEdProcesses.size()-1).getBurstTime();
                        LargeProcess = RemainEdProcesses.get(RemainEdProcesses.size()-1);
                        System.out.println("found, Varlargerjob = " + VarLargejob);
                    }
                }
                ...
                PreviousProcess = processes.get(Finished_Processes);
